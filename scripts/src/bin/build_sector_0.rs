...
use bios_bootloader_common::{PAGE_TABLE_1G, PAGE_TABLE_256T, PAGE_TABLE_512G, SECTOR_1};

fn main() {
    let util_len = fs::metadata("build/util.bin").unwrap().len();
    let mut elf =
        ElfStream::<LittleEndian, _>::open_stream(File::open("build/small_stage").unwrap())
            .unwrap();
    ...
    command.arg("bios_bootloader_small_stage/sector_0.nasm");
