...
const FREE_USED_PHYS_MEM_NODES_CAPACITY_MIN: usize = 2;
const FREE_USED_PHYS_MEM_NODES_CAPACITY: usize = 100;
const FREE_PAGE_TABLES_CAPACITY_MIN: usize = 4;
const FREE_PAGE_TABLES_CAPACITY: usize = 100;
...
impl MemoryData {
    ...
    fn refill_free_used_phys_mem_nodes(&mut self) {
        log::trace!("refilling");
        let used_phys_mem_node_phys_addr = self.free_used_phys_mem_nodes.pop().unwrap();
        let nodes_to_allocate = self.free_used_phys_mem_nodes.spare_capacity_mut().len();
        let alloc_size = (nodes_to_allocate * size_of::<LinkedListNode<UsedPhysMemNodeData>>())
            .try_into()
            .unwrap();
        let new_nodes_phys_addr = self
            .find_free_phys_mem(
                alloc_size,
        ...
        log::trace!("Allocating phys addr {new_nodes_phys_addr:#X} for used phys mem nodes.");
        let mut node_ptr = NonNull::new(
            (BIG_STAGE_MAP_OFFSET + used_phys_mem_node_phys_addr)
        ...
        let node_ptr = &raw mut *node_ptr.write(LinkedListNode::new(UsedPhysMemNodeData {
            phys_start: new_nodes_phys_addr,
            len: alloc_size,
        }));
        self.used_phys_mem_list
            .insert_sorted_boxed(unsafe { Box::from_raw(node_ptr) });
    ...
    fn alloc(&mut self, size: u64, align: u64) -> Option<u64> {
        if self.free_used_phys_mem_nodes.len() == 1 {
            self.refill_free_used_phys_mem_nodes();
        }

        let suitable_phys_start = self.find_free_phys_mem(size, align)?;
        log::trace!("suitable phys start: {suitable_phys_start:#X}");

        let node = {
            let mut node_ptr: NonNull<MaybeUninit<LinkedListNode<UsedPhysMemNodeData>>> =
                NonNull::new(
                    (BIG_STAGE_MAP_OFFSET + self.free_used_phys_mem_nodes.pop().unwrap())
                        as *mut MaybeUninit<LinkedListNode<UsedPhysMemNodeData>>,
                )
                .unwrap();
            let node_ptr = unsafe { node_ptr.as_mut() };
        ...
        self.used_phys_mem_list.insert_sorted_boxed(node);

        Some(suitable_phys_start)
    }
}
...
unsafe impl talc::source::Source for MySource {
    ...
    fn acquire<B: talc::base::binning::Binning>(
        ...
        let min_alloc_size = 0x200000;
        let phys_start = MEMORY
            .data
            .get()
            .unwrap()
            .lock()
            .alloc(
                u64::try_from(layout.size())
                    .unwrap()
                    .next_multiple_of(min_alloc_size),
                align,
            )
            .ok_or(())?;

        // FIXME: Actually map
        unsafe { talc.claim((BIG_STAGE_MAP_OFFSET + phys_start) as *mut _, size) };
        Ok(())
    }
}
...
impl Memory {
    ...
    pub fn map(&self, phys_addr: u64, len: u64, pat_index: u3) -> Result<u64, MapError> {
        let mut data = self.data.get().unwrap().lock();
        let virt_start =
            u64::try_from(data.virt_mem.alloc().ok_or(MapError::OutOfVirtMem)?).unwrap();

        let size = LeafMappingSize::_1G;
        unsafe {
            data.top_page_table.map_leaf(
                LeafMapping::new(
                    size,
                    virt_start.try_into().unwrap(),
                    phys_addr / size.byte_size() * size.byte_size(),
                ),
                &mut iter::from_fn(|| {
                    Some(unsafe { ScratchPageTable::new(data.free_page_tables.pop()?) })
                }),
                pat_index,
            )
        };
    ...
    /// Allocates 2 GiB of memory aligned to 2 GiB.
    pub fn alloc_virt(&self) -> Option<u64> {
        Some(
            self.data
                .get()
                .unwrap()
...
pub unsafe fn init(info: &BigStageEntryInfo) {
    ...
    let free_used_phys_mem_nodes = heapless::Vec::from([
    ...
    let free_page_tables = heapless::Vec::from_iter(
    ...
    let memory = MemoryData {
        ...
        top_page_table: pt,
        virt_mem: {
            let mut virt_mem = BitAlloc64K::default();
            log::info!("CAP: {:#X}", BitAlloc64K::CAP);
            // Mark lower 64 TiB as usable
            virt_mem.insert(0..0x8000);
            virt_mem
        },
    };
...
#[global_allocator]
static TALCK: talc::sync::TalcLock<spinning_top::RawSpinlock, MySource, DefaultBinning> =
    TalcLock::new(MySource);
